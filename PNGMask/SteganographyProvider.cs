...
namespace PNGMask
{
    public abstract class SteganographyProvider
    {
        ...
        public SteganographyProvider(Stream svector, string password, bool find = true)
        {
            ...
            ProcessData(buffer, password, find);
        }
        public SteganographyProvider(string fvector, string password, bool find = true)
        {
            if (!File.Exists(fvector)) throw new IOException("File '" + fvector + "' could not be found");

            ProcessData(File.ReadAllBytes(fvector), password, find);
        }

        public SteganographyProvider(byte[] bvector, string password, bool find = true) { 
            ProcessData(bvector, password); 
        }

        //
        public SteganographyProvider(PNG pngvector, string password, bool find = true)
        {
            byte[] data;
            ...
            ProcessData(data, password, find);
        }

        public abstract void ProcessData(byte[] s, string password, bool find = true);
        ...
        public void Imprint(DataType type, object obj, string password) //good at this point but data is changed in here somewhere
        {
            ...
            data = temp.ToArray();
            temp = null;
            ImprintData(data, password);
        }
        public abstract void ImprintData(byte[] data, string password);

        public DataType Extract(out object data, string password)
        {
            byte[] bdata = ExtractData(password);
        ...
        public abstract byte[] ExtractData(string password);
